...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    const credential = args.provider
      ? credentials.find(
          (item) =>
            item.key === args.provider ||
            database[item.key]?.name?.toLowerCase() === args.provider?.toLowerCase(),
        )
      : credentials[yield* promptValue(yield* Prompt.autocomplete({ message: "Select provider", maxItems: 8, options }))]
